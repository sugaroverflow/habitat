...
function localReply(message: string, roomId?: string | null) {
  ...
  if (
    ...
    return "TCL A400 Pro remains the model favourite.\n55 inch — about 122cm wide and £749 at the latest retail check. This is still the composition lean because it preserves more room for the gallery wall.\n65 inch — about 145cm wide and £1,099. Test it on the actual wall; pay the extra £350 only if the viewing distance or room scale makes 55 inches look genuinely undersized.\n\nBefore deciding, measure the clear window-to-heater width, heater position, floor-to-ceiling height, sofa viewing distance, and preferred TV/soundbar bottom edge. Confirm the exact retailer batch includes the ultra-slim mount.";
  }
  if (
    roomId === "living_room" &&
  ...
  if (text.includes("measure")) {
    return "Tomorrow's critical checks are: bathroom door, cabinet panel, surrounding wall edges and diagonal ceiling; relocated wardrobe footprint and carcasses; the full bedroom office-corner rectangle; the complete bedroom window wall; the living-room TV wall, heater, ceiling, viewing distance and preferred TV/soundbar height; and a 130 × 100cm dining-table footprint with four chairs and the island route.\n\nSecondary checks are the bedroom bed wall and obstructions, Magnus Pro size, divider depth and height, chair and bed clearances, ceiling height, plus any small bathroom returns that should become Mineral Green. The full grouped checklist is on the Measurements page, linked from Home.";
  }
